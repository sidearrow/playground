...
const Component: React.FC<{
  ...
  const hasBranch = lineDetail.stations.findIndex(v => v.branchLineName !== null) !== -1;
        ...
        <table>
          ...
          <tbody>
            ...
            {stations.map((station, i) => (
              <>
                {station.branchLineName !== null && stations[i - 1].branchLineName === null && (
                  <tr className="divide">
                    <td colSpan={6} className="fw-bold">{station.branchLineName}</td>
                  </tr>
                )}
                <tr className={i > 1 && stations[i - 1].branchLineName !== null && station.branchLineName === null ? 'divide' : ''}>
                  <td>{station.stationName}</td>
                  <td>{station.stationNameKana}</td>
                  <td className="text-right">{station.lengthBetween === null ? '' : station.lengthBetween.toFixed(1)}</td>
